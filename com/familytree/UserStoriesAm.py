...
class UserStoriesAm:
    ...
    def us02(self):
        ...
        processed_tree = TreeLine().process_data(UserStoriesAm.FILE_PATH)
        ...
        for indi in indi_list:
            ...
            for spouse in indi.fams:
                ...
                if marriage_date and birth_date > marriage_date:
                    indi.warn_msg = f"birth date should not occur after marriage date"
            ...
            if divorce_date:
                if husband_death_date and wife_death_date:
                    if divorce_date > husband_death_date and divorce_date > wife_death_date:
                        fam.warn_msg = f"death date is before the divorce date for both"
                    ...
                    if husband_death_date and divorce_date > husband_death_date:
                        fam.warn_msg = f"death date of husband is  before the divorce date"
                        indi_list_us06_fail.append(fam)
                    if wife_death_date and divorce_date > wife_death_date:
                        fam.warn_msg = f"death date of wife is  before the divorce date"
                        indi_list_us06_fail.append(fam)
                else:
                    continue   
